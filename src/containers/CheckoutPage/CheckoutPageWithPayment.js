...
export const CheckoutPageWithPayment = props => {
  ...
  // ✅ REAL Money (subunits + currency) from the transaction
  const totalPriceMoney =
    tx?.attributes?.payinTotal ||
    tx?.attributes?.totalPrice ||
    tx?.attributes?.payoutTotal ||
    null;

  // ✅ UI string (keep for display only)
  const totalPrice =
    totalPriceMoney ? formatMoney(intl, totalPriceMoney) : null;
            ...
            {showPaymentForm ? (
              <div className={css.paymentForm}>
                <button
                  type="button"
                  onClick={async () => {
                    const totalAmountNumber = totalPriceMoney ? totalPriceMoney.amount / 100 : null;

                    if (!totalAmountNumber) {
                      alert('Cannot determine total amount');
                      return;
                    }
                    ...
                    const res = await fetch('/api/hubtel-initiate', {
                      ...
                      body: JSON.stringify({
                        description: `Order for ${listing.attributes.title}`,
                        totalAmount: totalAmountNumber,
                        listingId: listing.id.uuid,
                        lineItems: tx?.attributes?.lineItems || [],
                        stockReservationQuantity: orderData?.quantity || orderData?.seats || 1,
                      }),
                    });
                    ...
                    if (!res.ok) {
                      console.error('hubtel-initiate failed:', data);
                      alert(data?.error || 'Hubtel initiate failed. Check Network → hubtel-initiate → Response.');
                      return;
                    }
                    ...
                    const redirectUrl =
                      ...
                      data?.checkoutUrl ||
                      data?.hubtel?.data?.checkoutDirectUrl ||
                      data?.hubtel?.data?.checkoutUrl;

                    if (!redirectUrl) {
                      console.error('No checkout URL in response:', data);
                      alert('Hubtel did not return a checkout URL. Open Network → hubtel-initiate → Response.');
                      return;
                    }
